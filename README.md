# Подытожим, выражения теперь хранятся в БД, есть возможность регистрации, авторизации, но без аунтификации, я хз как из токена понимать userID для выражения, агент с оркестратором общается по gRPC(было легко реализовать), чтобы проект был полноценным надо понимать какой user обращается, вообщем проект рабочий только на 50%, ставьте баллы как знаете. Важный урок: не надо откладывать все на последний день. У меня всё
